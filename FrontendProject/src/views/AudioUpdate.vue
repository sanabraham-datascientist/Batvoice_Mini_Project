      ...
      <div v-for="(segment, id) in audio.segments" :key="id">
        <div>
          <div class="flex flex-row justify border-b-2 border-gray-200">
            ...
            <div  v-if="segments_validation_errors[id]">
                <div 
                 
                  class="bg-red-300 text-white rounded-lg p-6 "
                > <p>{{segments_validation_errors[id]}}</p>
...
export default {
  ...
  data() {
    return {
      ...
      segments_validation_errors:[],

      rules: [
        { 1: "All characters belong to a given character set" },
        {
          2: "Capital letters are allowed only as a first word letter or if all the letters in the word are upper case",
        },
        { 3: "There can be only zero or one space between two characters" },
        {
          4: "characters ?.! should be end of text or followed by one space and an uppercase character",
        },
        { 5: 'characters ,;: should be end of text of followed by one space"' },
      ],
    };
  },
  created() {
  ...
  methods: {
    ...
    async updateAudio() {
      ...
      await axios
        .patch(`/api/audios/${id}/update/`, this.audio)
        .then((response) => {
          this.getAudio();
        ...
        .catch((error) => {
          this.segments_validation_errors = error.response.data.segments
          console.log(error.response.data.segments)
        });
    },
  },
};
